...
async function openTraining(id){
  ...
  const h=document.createElement("h2");h.textContent=title;const p=document.createElement("p");p.textContent=description||"";p.style.lineHeight="1.9";
  card.append(close,h,p);
  if(q.video_url){const box=document.createElement("div");box.style.marginTop="14px";const yt=q.video_url.match(/(?:youtube\.com\/(?:watch\?v=|embed\/)|youtu\.be\/)([^?&/]+)/i);if(yt){const iframe=document.createElement("iframe");iframe.src="https://www.youtube.com/embed/"+encodeURIComponent(yt[1]);iframe.title=title;iframe.allowFullscreen=true;iframe.loading="lazy";iframe.style.cssText="width:100%;aspect-ratio:16/9;border:0;border-radius:14px";box.appendChild(iframe);}else{const a=document.createElement("a");a.className="btn";a.target="_blank";a.rel="noopener";a.href=q.video_url;a.textContent=T("watch");box.appendChild(a);}card.appendChild(box);}else{const p2=document.createElement("p");p2.className="muted small";p2.textContent=T("noVideo");card.appendChild(p2);}
  if(q.pdf_url){const a=document.createElement("a");a.className="btn alt";a.target="_blank";a.rel="noopener";a.href=q.pdf_url;a.textContent="📄 "+T("openPdf");card.appendChild(a);}else{const p3=document.createElement("p");p3.className="muted small";p3.textContent=T("noPdf");card.appendChild(p3);}
  const done=document.createElement("button");done.className="btn";done.style.marginTop="14px";done.textContent=T("finish");done.onclick=async()=>{modal.remove();await finishTraining(id);};card.appendChild(done);modal.appendChild(card);document.body.appendChild(modal);modal.addEventListener("click",e=>{if(e.target===modal)modal.remove();});
}
