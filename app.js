...
const tr = {
  ar: {
    ...
    changeStatus:"تغيير الحالة", suspendMember:"إيقاف دخول العضو", reactivateMember:"إعادة تفعيل العضو", suspendedMessage:"تم إيقاف حسابك. يرجى التواصل مع الإدارة.", ranking:"ترتيب النشاط والإنجاز", activityScore:"مؤشر النشاط", addFollowup:"إضافة متابعة", saveMember:"حفظ العضو", adminOnly:"للمسؤول فقط",
  ...
  fr: {
    ...
    changeStatus:"Changer le statut", suspendMember:"Suspendre l’accès", reactivateMember:"Réactiver le membre", suspendedMessage:"Votre compte est suspendu. Contactez l’administration.", ranking:"Classement activité et progression", activityScore:"Indice d’activité", addFollowup:"Ajouter un suivi", saveMember:"Enregistrer le membre", adminOnly:"Administrateur seulement",
...
async function boot(){
  ...
  member=m;isAdmin=Boolean(a); if(!member){await db.auth.signOut();auth("login",lang==="ar"?"لم يتم العثور على ملف العضو":"Profil membre introuvable");return;} if(isAdmin){renderAdminOnly();return;} if(member.status==="inactive"){await db.auth.signOut();member=null;isAdmin=false;auth("login",T("suspendedMessage"));return;} await dashboard();
}
...
async function adminMembers(body){
  const [{data:members,error:e1},{data:points,error:e2},{data:tasks,error:e3},{data:training,error:e4},{data:activity,error:e5}]=await Promise.all([
    db.from("dxn_start_members").select("*").order("created_at",{ascending:false}),
    db.from("dxn_start_points").select("member_id,points,created_at"),
    db.from("dxn_start_task_completions").select("member_id,completed_at"),
    db.from("dxn_start_progress").select("member_id,completed,completed_at"),
    db.from("dxn_start_activity").select("member_id,created_at")
  ]);
  const error=e1||e2||e3||e4||e5;if(error){body.innerHTML=`<div class="card">${esc(errText(error))}</div>`;return;}
  const all=members||[], stats=new Map();
  all.forEach(x=>stats.set(x.id,{points:0,tasks:0,training:0,activity:0,last:""}));
  (points||[]).forEach(x=>{const z=stats.get(x.member_id);if(z){z.points+=Number(x.points)||0;if((x.created_at||"")>z.last)z.last=x.created_at||"";}});
  (tasks||[]).forEach(x=>{const z=stats.get(x.member_id);if(z){z.tasks++;if((x.completed_at||"")>z.last)z.last=x.completed_at||"";}});
  (training||[]).forEach(x=>{const z=stats.get(x.member_id);if(z&&x.completed){z.training++;if((x.completed_at||"")>z.last)z.last=x.completed_at||"";}});
  (activity||[]).forEach(x=>{const z=stats.get(x.member_id);if(z){z.activity++;if((x.created_at||"")>z.last)z.last=x.created_at||"";}});
  all.forEach(x=>{const z=stats.get(x.id);z.score=z.points+(z.training*3)+(z.tasks*2)+z.activity;});
  all.sort((a,b)=>{const A=stats.get(a.id),B=stats.get(b.id);return B.score-A.score||B.points-A.points||B.training-A.training||B.tasks-A.tasks||(B.last||"").localeCompare(A.last||"")||String(a.full_name||"").localeCompare(String(b.full_name||""));});
  body.innerHTML=`<div class="card"><div class="small muted" style="margin-bottom:8px">🏆 ${T("ranking")}</div><input id="adminSearch" style="width:100%;padding:12px;border:1px solid #ccd6df;border-radius:10px" placeholder="${T("search")}"><div id="adminMembers" class="list" style="margin-top:12px"></div></div>`;
  const list=body.querySelector("#adminMembers");
  const draw=()=>{const q=(body.querySelector("#adminSearch").value||"").toLowerCase();const rows=all.filter(x=>[x.full_name,x.dxn_member_id,x.whatsapp,x.city,x.status].join(" ").toLowerCase().includes(q));list.innerHTML=rows.map((x,i)=>{const z=stats.get(x.id);return `<div class="item"><div class="row between"><b>${i+1}. ${esc(x.full_name)}</b><span class="pill">${esc(x.status)}</span></div><div class="small muted">${esc(x.dxn_member_id||"—")} · ${esc(x.whatsapp||"—")} · ${esc(x.city||"—")}</div><div class="small" style="margin-top:6px">⭐ ${T("points")}: ${z.points} · 🎓 ${z.training} · ✅ ${z.tasks} · 🔥 ${z.activity}</div><div class="row" style="margin-top:8px"><button class="btn alt" data-member="${x.id}">${T("details")}</button>${x.whatsapp?`<a class="btn" target="_blank" rel="noopener" href="https://wa.me/${String(x.whatsapp).replace(/\D/g,"")}">${T("send")}</a>`:""}</div></div>`;}).join("")||T("no");list.querySelectorAll("[data-member]").forEach(b=>b.onclick=()=>memberEditor(b.dataset.member,all,body));};
  body.querySelector("#adminSearch").oninput=draw;draw();
}

function memberEditor(id,all,body){const x=all.find(z=>z.id===id);if(!x)return;const suspended=x.status==="inactive";const card=document.createElement("div");card.className="card";card.style.marginTop="12px";card.innerHTML=`<h3>${T("details")}: ${esc(x.full_name)}</h3><div class="field"><label>${T("name")}</label><input id="mname" value="${esc(x.full_name)}"></div><div class="field"><label>${T("age")}</label><input id="mage" type="number" min="13" max="100" value="${Number(x.age)||""}"></div><div class="field"><label>${T("wa")}</label><input id="mwa" value="${esc(x.whatsapp||"")}"></div><div class="field"><label>${T("id")}</label><input id="mid" value="${esc(x.dxn_member_id||"")}"></div><div class="field"><label>${T("city")}</label><input id="mcity" value="${esc(x.city||"")}"></div><div class="field"><label>${T("status")}</label><select id="mstatus"><option value="new">new</option><option value="active">active</option><option value="needs_followup">needs_followup</option><option value="inactive">inactive</option><option value="completed">completed</option></select></div><div class="field"><label>${T("notes")}</label><textarea id="mnotes" rows="4">${esc(x.notes||"")}</textarea></div><div class="row"><button class="btn" id="saveM">${T("saveMember")}</button><button class="btn alt" id="cancelM">${T("cancel")}</button><button class="btn alt" id="toggleM">${suspended?T("reactivateMember"):T("suspendMember")}</button></div>`;body.prepend(card);card.querySelector("#mstatus").value=x.status||"new";card.querySelector("#cancelM").onclick=()=>card.remove();card.querySelector("#saveM").onclick=async()=>{const payload={full_name:card.querySelector("#mname").value.trim(),age:Number(card.querySelector("#mage").value)||null,whatsapp:card.querySelector("#mwa").value.trim(),dxn_member_id:card.querySelector("#mid").value.trim(),city:card.querySelector("#mcity").value.trim(),status:card.querySelector("#mstatus").value,notes:card.querySelector("#mnotes").value.trim()};const {error}=await db.from("dxn_start_members").update(payload).eq("id",id);if(error){toast(errText(error));return;}toast(T("memberSaved"));adminMembers(body);};card.querySelector("#toggleM").onclick=async()=>{const next=suspended?"active":"inactive";if(!confirm(suspended?(lang==="ar"?"إعادة تفعيل دخول هذا العضو؟":"Réactiver l’accès de ce membre ?"):(lang==="ar"?"إيقاف دخول هذا العضو؟ سيمنع من دخول المنصة.":"Suspendre l’accès de ce membre ? Il ne pourra plus se connecter.")))return;const {error}=await db.from("dxn_start_members").update({status:next}).eq("id",id);if(error){toast(errText(error));return;}toast(T("success"));adminMembers(body);};}
